...
ylim = [plt.gca().axes.get_ylim()[0], plt.gca().axes.get_ylim()[1]]
for i in range(len(vlines['title'])):
    ...
    plt.vlines(x=datetime.strptime(vlines['date'][i], '%Y-%m-%d'), ymin=ylim[0], ymax=ylim[1], color='k', linestyles='dashed', linewidth=0.5)
